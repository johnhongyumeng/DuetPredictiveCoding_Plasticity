...
flag_homeo=0
flag_align=1
Fig_folder='\\figs\\plasticity_saturate_noHomeo_test'
N_eachC=200
...
r_saturate=3
totalTime = 20.01+6 # Default 1000s.+6.
jitter=0.0
    ...
    if i_time % save_interval == 0 and i_frames < n_frames:
        ...
        if not(i_time == 0):  # learning rate, varying the connectivity matrix.
            ...
            if not(np.isnan(learning_protocol)) and i_frames<n_frames-6 and i_frames>6:
                dWEds = -(learning_error-0.5)*alpha*((E_pop.rate-E_baseline)*S_pop.rate )
...
PltTool.Plot_var_Snapshots(Output_dic['E_pop_rate'][:,[3,4,2,5,1]],
                           labels=['Stim','Pred','Expected','Trained Stim','Baseline',],
                           linestyles=['g','b','k','r','--k'],
                           name='InitalSnapshots')
PltTool.Plot_var_Snapshots(Output_dic['E_pop_rate'][:,[n_frames-2,n_frames-1,n_frames-3,n_frames-4,n_frames-6]],
                           labels=['Stim','Pred','Expected','Trained Stim','Baseline',],
                           linestyles=['g','b','k','r','--k'],
