# -*- coding: utf-8 -*-
"""
Modified on 11112024. Now I notice that I need two SST population for each stimulus. So let's do it
Second thoughts, the pyramidal cells should receive two types of excitation again. This is serious matching
to David Schneider's work. Let's keep working. Now the pyramidal cells should receive two kinds of 
input. So if that's the case, the prediction cannot be aligned to either one of them. 
This is a serious change of the coding structure. Tuning and debugging will take a week. But it will worth it.
Let's start coding. Keep pushing, John
I'm good. This is done. Pretty clean modeling result. OK, let's generate figures for these three figures. 
Now I really need to see how to organize these stuff.
...
#misAlign  N   In this case, flag_align is always 0
flag_homeo=0
flag_align=0
...
r_saturate=3
totalTime = 6.01+5 # First and Last 2.5s for testing. Put it here for easy modification
jitter=0.0
...
params_S2 = PARAS['PARAMS_SST'].copy()
...
# The following is to balace top-down effect. Actually. This inc_back should be put into the dendrite. 
# If I have to change the learning again, I will change it. 
...
S2_pop= data_structure.Interneuron(params_stim['N_neu'])
...
W_dict['EdS2'] = bf.create_random_delta_connectivity(
    params_S['weight_to_dend'], N_neu, sigma_r=sigma_W, rng=rng3)
...
W_dict['S2Int']= np.zeros((N_neu,N_column))
...
Output_dic= dict(
    ...
    S2_pop_rate= np.full((N_neu,n_frames),np.nan, dtype=np.float32) ,
    S2_pop_sumI= np.full((N_neu,n_frames),np.nan, dtype=np.float32) ,
    ...
    g_S2_E=np.full((N_neu,n_frames),np.nan, dtype=np.float32) ,
    g_S2_EN=np.full((N_neu,n_frames),np.nan, dtype=np.float32) ,
    g_S2_I=np.full((N_neu,n_frames),np.nan, dtype=np.float32) ,
    ...
    W_EdS2=np.full((N_neu, n_frames), np.nan, dtype=np.float32),

    W_EP=np.full((N_neu, n_frames), np.nan, dtype=np.float32),
    dWEds=np.full((N_neu, n_frames), np.nan, dtype=np.float32),
    dWEds2=np.full((N_neu, n_frames), np.nan, dtype=np.float32),
...
LHS_update_S2= update_functions.LHS_update.S(params_S2,params_sim['dt'])

# Including the 2nd stimulus input here to keep the RNG sequence as before for reproductibility.
shuffle_stim_indices = np.random.permutation(N_eachC)
stimuli2_inc_learn=stimuli_inc_learn[shuffle_stim_indices]
...
#for i_time in tqdm(range(int(3/params_sim['dt']))):    
    ...
    if (i_time-1) % save_interval == 0 and i_frames < n_frames and i_time > save_interval:
        ...
        if learning_protocol == 0:   # Matching learning. Needs to improve more
            ...
            Int_pop.rate=10.0
        '''
        if learning_protocol>=0:  # under predict. Stim> pred  flag_algin is alway 0
            RHS_update.g_back['Es'] = stimuli_inc_learn+g_back['Es']
            RHS_update.g_back['S'] = stimuli_ave_learn*Input_S_ratio+g_back['S']
            RHS_update.g_back['S2'] = g_back['S2']
                
            if i_frames>=n_frames-3 or i_frames<=3: # For testing the output
                RHS_update.g_back['Es'] = stimuli2_inc_learn+g_back['Es']
                RHS_update.g_back['S2'] = stimuli_ave_learn*Input_S_ratio+g_back['S2']
                RHS_update.g_back['S'] = g_back['S']                    
            ...
            RHS_update.g_back['S2'] = g_back['S2']
            if i_frames>=n_frames-3 or i_frames<=3:  # During testing, the S2 gets input, but not S
                RHS_update.g_back['Es'] = stimuli2_inc_learn*(1-learning_error)+g_back['Es']
                RHS_update.g_back['S2'] = stimuli_ave_learn*Input_S_ratio*(1-learning_error)+g_back['S2']
                RHS_update.g_back['S'] = g_back['S']                
            Int_pop.rate=10.0    
    ...
    E_pop.gI_dend= RHS_update.g_pyr_dendI2(S_pop.h*WsInhFlag, S2_pop.h)    
    ...
    S2_pop.gE= RHS_update.g_sst2_E(E_pop.h, Int_pop.h)   
    S2_pop.gEN= RHS_update.g_sst2_EN(E_pop.hN, Int_pop.hN)    
    S2_pop.gI= RHS_update.g_sst_I(P_pop.h) 
    ...
    S2_pop.h= LHS_update_S2.h(S2_pop.h,S2_pop.rate)
    ...
    S2_pop.rate, S2_total_current=LHS_update_S2.rate(S2_pop )
    ...
    if i_time % save_interval == 0 and i_frames < n_frames:
        ...
        W_EdS2 = np.diag(W_dict['EdS2']).copy()
        ...
        if not(i_time == 0):  # learning rate, varying the connectivity matrix.
            ...
            if not(np.isnan(learning_protocol)) and i_frames< n_frames-6 and i_frames>6:
                dWEds = -(learning_error-0.5)*eta_S*alpha*((E_pop.rate-E_baseline)*S_pop.rate )
                dWEds2 = -(learning_error-0.5)*eta_S*alpha*((E_pop.rate-E_baseline)*S2_pop.rate )

                dWEP =  -(learning_error-0.5)*alpha*((E_pop.rate-E_baseline)*P_pop.rate )
                if learning_error>0.5:
                    dWEds = dWEds* bf.relu(-np.mean(E_pop.rate)+gamma_saturate*E_baseline)/gamma_saturate/E_baseline
                    dWEds2 = dWEds2* bf.relu(-np.mean(E_pop.rate)+gamma_saturate*E_baseline)/gamma_saturate/E_baseline

                    dWEP = dWEP* bf.relu(-np.mean(E_pop.rate)+gamma_saturate*E_baseline)/gamma_saturate/E_baseline
                    
               
            #if not(np.isnan(learning_protocol)) and i_frames<n_frames-6 and i_frames>5:
                W_EdS += dWEds* (r_saturate*mean_EdS_value-W_EdS)/mean_EdS_value/(r_saturate-1)
                W_EdS2 += dWEds2* (r_saturate*mean_EdS_value-W_EdS2)/mean_EdS_value/(r_saturate-1)
                
                W_EP  += dWEP* (r_saturate*mean_EP_value-W_EP)/mean_EP_value/(r_saturate-1)
                W_dict['EdS'] = np.diag(bf.relu(W_EdS))
                W_dict['EdS2'] = np.diag(bf.relu(W_EdS2))
        ...
        Output_dic['W_EdS2'][:, i_frames] = np.diag(W_dict['EdS2']).copy()
        Output_dic['W_EP'][:, i_frames] = np.diag(W_dict['EP']).copy()
        Output_dic['dWEds'][:, i_frames] = dWEds
        Output_dic['dWEds2'][:, i_frames] = dWEds2
        ...
        Output_dic['S2_pop_rate'][:,i_frames]=S2_pop.rate
        Output_dic['S2_pop_sumI'][:,i_frames]=S2_total_current
        ...
        Output_dic['g_S2_E'][:,i_frames]=S2_pop.gE
        Output_dic['g_S2_EN'][:,i_frames]=S2_pop.gEN
        Output_dic['g_S2_I'][:,i_frames]=S2_pop.gI
...
'''
# Maybe modifying into two SST subtypes are necessary. But let's keep it this way. 
# Maybe also test on the nPE test code as well.
...
PltTool.PlotPopImshow(Output_dic['W_EdS2'], 'W_EdS2_')
...
PltTool.PlotPopImshow(Output_dic['dWEds2'], 'dWEds2_')
...
PltTool.Plot_Dict(Output_dic['S2_pop_rate'][0, :], 'S2popRate_all')
...
PltTool.Plot_2Snapshots(Output_dic['W_EdS2'][:,1],Output_dic['W_EdS2'][:,n_frames-1],'W_EdS2Learning')
...
PltTool.Plot_FixframeInt(Output_dic['W_EdS'],10,name='W_Eds_frames')
PltTool.Plot_FixframeInt(Output_dic['W_EdS2'],10,name='W_Eds2_frames')
PltTool.Plot_FixframeInt(Output_dic['W_EP'],10,name='W_EP_frames')
...
PltTool.Plot_Snapshots(Output_dic['E_pop_rate'][:,2],Output_dic['E_pop_rate'][:,3],
                       Output_dic['E_pop_rate'][:,4],Output_dic['E_pop_rate'][:,1],
                       'Initial')
PltTool.Plot_Snapshots(Output_dic['E_pop_rate'][:,n_frames-3],Output_dic['E_pop_rate'][:,n_frames-2],
                       Output_dic['E_pop_rate'][:,n_frames-1],Output_dic['E_pop_rate'][:,1],
                       'Final')
...
PltTool.Plot_var_Snapshots(Output_dic['E_pop_rate'][:,[n_frames-2,n_frames-1,n_frames-3,n_frames-4,n_frames-5,1]],
                           labels=['Stim','Pred','Expected','Trained Stim','Trained Expect','Baseline',],
                           linestyles=['g','b','k','r','grey','--k'],
                           name='LearnedSnapshots')
PltTool.Plot_var_Snapshots(Output_dic['E_pop_rate'][:,[n_frames-2,n_frames-1,n_frames-3,n_frames-4,n_frames-5,n_frames-6]],
                           labels=['Stim','Pred','Expected','Trained Stim','Trained Expect','Baseline',],
                           linestyles=['g','b','k','r','grey','--k'],
                           name='LearnedSnapshots')
...
PltTool.Plot_var_Snapshots(Output_dic['E_pop_rate'][:,[n_frames-3,n_frames-5]],
                           labels=['Expected','Trained Expect'],
                           linestyles=['g','grey'],
                           name='LearnedExpectedSnapshots')


rates=[np.mean(Output_dic['E_pop_rate'][:,6]),np.mean(Output_dic['E_pop_rate'][:,5]),
...
rates=[np.mean(Output_dic['E_pop_rate'][:,n_frames-5]),np.mean(Output_dic['E_pop_rate'][:,n_frames-4]),
     np.mean(Output_dic['E_pop_rate'][:,n_frames-1]),np.mean(Output_dic['E_pop_rate'][:,n_frames-6])]
...
PltTool.Plot_overlay_hist(Output_dic['E_pop_rate'][:,2],Output_dic['E_pop_rate'][:,n_frames-3],
                          ...
                          name='Stim2+Pred',bin_width=1.0)
PltTool.Plot_overlay_hist(Output_dic['E_pop_rate'][:,6],Output_dic['E_pop_rate'][:,n_frames-5],
                          start_color='grey',end_color='#e34a33',labels=('Pre','Post'),
                          name='Stim+Pred',bin_width=1.0)
...
PltTool.Plot_overlay_hist(Output_dic['E_pop_rate'][:,5],Output_dic['E_pop_rate'][:,n_frames-4],
                          ...
                          name='Trained Stim',bin_width=1.0)
import AnalysisTool as AT
PltTool=AT.Tools(Output_dic, ndt,n_frames, params_sim['dt'], params_stim, 
...
para_save['shuffle_stim_indices']=shuffle_stim_indices
