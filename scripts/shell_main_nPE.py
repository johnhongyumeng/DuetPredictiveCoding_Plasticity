...
from class_main_nPE_perturbe import nPE_model 
...
#case=0
SST_pert_array=np.arange(-1,1.01,0.1)
#case=1
PV_pert_array=np.arange(-2,2.01,0.2)
#case=2
NMDA_pert_array=np.arange(0,1.01,0.1)

if case==0:
    fig_name='DifOverSSTnPEtest'
elif case==1:
    fig_name='DifOverPVnPEtest'
elif case==2:
    fig_name='DifOverNMDAnPEtest'
...
for ite_pert in NMDA_pert_array:
    nPE_test=nPE_model()
    if case==0:
        DD_temp,Output_dic,PltTool=nPE_model.main(ratio_SST_perturb=ite_pert,Plot_flag=False)
    elif case==1:
        DD_temp,Output_dic,PltTool=nPE_model.main(ratio_PV_perturb=ite_pert,Plot_flag=False)
    elif case==2:
        DD_temp,Output_dic,PltTool=nPE_model.main(ratio_NMDA_perturb=ite_pert,Plot_flag=False)
...
fig.savefig('../figs/looped/nPE/'+fig_name+ 'pop.jpg',bbox_inches='tight')
fig.savefig('../figs/looped/nPE/'+fig_name+ 'pop.svg',bbox_inches='tight')
...
axes.spines["top"].set_visible(False)
axes.spines["right"].set_visible(False)
fig.savefig('../figs/looped/nPE/'+fig_name+ 'IndNeu.svg',bbox_inches='tight')
axes.set_ylabel('nPE and pPE neuron dR')
fig.savefig('../figs/looped/nPE/'+fig_name+ 'IndNeu.jpg',bbox_inches='tight')
