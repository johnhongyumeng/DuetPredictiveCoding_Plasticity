...
For a loop of InjRate. Change the InjDendrite and InjSoma in the rate code
@author: Hongyu Meng
"""
import numpy as np
...
fig_tag='InjSE8L23SEDE_'   # Seems 5 for L5, 8 for L23 is good.
coupling_conductance=  80     # Coupling 80 for L23, 10 for L5


#fig_tag='InjSEDEcurrentL5_'
#coupling_conductance=  10     # Coupling 80 for L23, 10 for L5

Num_Injd=11
g_Injd_min=150   # 0.1 nS for the generic VIP model
g_Injd_max=400   # The unit is pA
...
for i_ext in tqdm(range(Num_Injd)):
    for j_ext in range(Num_Injs):
        Injd_ite=g_Injd_vec[i_ext]
        Injs_ite=g_Injs_vec[j_ext]
        CondSE_ite = 0
        Condd_ite=0
        ConddI_ite=0
...
TempPlt=mycolormesh(g_Injs_vec,g_Injd_vec)
TempPlt.myplot(VoltD_mat.transpose(),'DendVolt',fig_tag,contour_value=4)
TempPlt.myplot(Isd_mat.transpose(),'Isd',fig_tag,contour_value=4)
#TempPlt.myplot(Itotal_mat.transpose(),'Itotal',fig_tag,contour_value=4)
TempPlt.myplot(Rate_mat.transpose(),'rate',fig_tag,vmin=0,contour_value=4)
