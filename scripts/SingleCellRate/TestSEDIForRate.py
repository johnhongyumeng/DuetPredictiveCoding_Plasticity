...
fig_tag='TestSEDIL23_'   # Seems 5 for L5, 8 for L23 is good.
...
Num_Injd=6
g_Injd_min=5   # 0.1 nS for the generic VIP model % More like x-axis.
g_Injd_max=15 
...
for i_ext in tqdm(range(Num_Injd)):
    for j_ext in range(Num_Injs):
        CondSE_ite=g_Injd_vec[i_ext]     # I know this is weird, but keep it here for now.
